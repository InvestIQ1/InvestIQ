...
export const ChooseCategory: React.FC = () => {
  ...
  const allExpensesArray: item[] = [
    {
      id: "1",
      money: selectorExpense
        .filter((action) => action.category === "Продукти")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Products,
      name: "Продукти",
    },
    {
      id: "2",
      money: selectorExpense
        .filter((action) => action.category === "Алкоголь")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Alcohol,
      name: "Алкоголь",
    },
    {
      id: "3",
      money: selectorExpense
        .filter((action) => action.category === "Здоров’я")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Health,
      name: "Здоров’я",
    },
    {
      id: "4",
      money: selectorExpense
        .filter((action) => action.category === "Розваги")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Fun,
      name: "Розваги",
    },
    {
      id: "5",
      money: selectorExpense
        .filter((action) => action.category === "Транспорт")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Transport,
      name: "Транспорт",
    },
    {
      id: "6",
      money: selectorExpense
        .filter((action) => action.category === "Все для дому")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: House,
      name: "Все для дому",
    },
    {
      id: "7",
      money: selectorExpense
        .filter((action) => action.category === "Техніка")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Machinery,
      name: "Техніка",
    },
    {
      id: "8",
      money: selectorExpense
        .filter((action) => action.category === "Комуналка, зв’язок")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Bills,
    ...
    },
    {
      id: "9",
      money: selectorExpense
        .filter((action) => action.category === "Спорт, хобі")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Hobby,
      name: "Спорт, хобі",
    },
    {
      id: "10",
      money: selectorExpense
        .filter((action) => action.category === "Навчання")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Learning,
      name: "Навчання",
    },
    {
      id: "11",
      money: selectorExpense
        .filter((action) => action.category === "Інше")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Ufo,
      name: "Інше",
    },
  ...
  const allIncomesArray: item[] = [
    {
      id: "12",
      money: selectorIncome
        .filter((action) => action.category === "ЗП")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Salary,
      name: "ЗП",
    },
    {
      id: "13",
      money: selectorIncome
        .filter((action) => action.category === "ДОД. ДОХІД")
        .reduce((acc, action) => acc + action.sum, 0),
      photo: Income,
      name: "ДОД. ДОХІД",
    },
  ...
  const handleClick = (id: string) => {
    ...
    const activeCategory: string | undefined = data.find(
      (item) => item.id === id,
    )?.name;
